...
namespace TflinkTest.FamilyTree
{
    public partial class View : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                mainmemidd = Session["Mainmemid"].ToString();
            ...
            string memidqrystrng = Request.QueryString["Memidview"];
            if (target != "photo")
            {
            ...
            try
            {
                ...
                MemberId = memidqrystrng.ToString();

                userid = getusernamebymemid(MemberId);
                // userid = Session["UserId"].ToString();
        ...
        public string getusernamebymemid(string MemberIdd)
        {
            string useridd = "";
            SqlConnection con = new SqlConnection(strcon);
            SqlDataAdapter da = new SqlDataAdapter("select * from MainMembers where Memberid='" + MemberIdd + "'", con);
            DataTable dt = new DataTable();
            da.Fill(dt);
            if (dt.Rows.Count > 0)
            {
                useridd = dt.Rows[0]["Userid"].ToString();
            }
        ...
        public void getmemdetails(string memid)
        {
            ...
            if (dt.Rows.Count > 0)
            {
                ...
                cicrabirth = dt.Rows[0]["Creradob"].ToString();
        ...
        public void Submitdata()
        {
            ...
            cmd.Parameters.AddWithValue("Enterby", mainmemidd);
            ...
            if (k != 0)
            {
                ...
                Session["Slno"] = null;
                Response.Redirect("../FamilyTree/View.aspx?Memidview=" + MemberId + "");
            }
            else
            {
        ...
        public string checkcurssion(string curssionmemid, string user_id)
        {
            ...
            if (dt1.Rows.Count > 0)
            {
                ...
                if (curmemid == mainmemidd || enterby == mainmemidd)
                {
        ...
        public void SubmitNames()
        {
            ...
            if (k != 0)
            {
                //Response.Write("<script>alert('Data Saved Successfully.')</script>");
                Session["Slno"] = null;
                Response.Redirect("../FamilyTree/View.aspx");
            }
            else
            {
                string line = "<div class='alert alert-danger'><strong> Success!</strong> Submit Failed</div> ";
        ...
        protected void Button1_Click1(object sender, EventArgs e)
        {
            ...
            if (ID == "" || ID != "")
            {
                ...
                SqlCommand cmd1 = new SqlCommand("insert into Tbl_Pictures values('" + familyid + "','" + MemberId + "','" + pictureid + "','" + img_showimg.ImageUrl + "','" + txt_Desc.Text.Trim() + "','" + date + "','" + "" + "','" + admin + "','" + date + "')", con);
        ...
        protected void btn_Paswordchng_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                //sqlDa.Fill(dt);
                con.Open();
                string Query = "select * from MainMembers where MemberId='" + MemberId + "' and Password='" + encryptedpswrd + "'";
                DataTable dt = RetriveData(Query);
                //  string ordersts = dt.Rows[0]["Status"].ToString();
                if (dt.Rows.Count > 0)
                {
                    SqlCommand cmd = new SqlCommand("Update MainMembers set Password='" + SomeStaticClass.encrypt(txt_Newpswrd.Text.Trim()) + "' where MemberId='" + MemberId + "'", con);
                    cmd.ExecuteNonQuery();
